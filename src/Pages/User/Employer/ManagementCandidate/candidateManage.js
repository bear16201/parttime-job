import Header from "../../Themes/Header/headerEmployer";
import SideBar from "../MangementPage/sidebar";
import Rating from '@mui/material/Rating'
import Button from 'react-bootstrap/Button';
import Form from 'react-bootstrap/Form';
import { Modal } from 'react-bootstrap';
import './candidateManage.scss'
import { useState } from 'react';
import { useEffect } from 'react';
import logo_job from '../../../../Assets/logo-job.png'
import Dialog from '@mui/material/Dialog';
import DialogActions from '@mui/material/DialogActions';
import DialogContent from '@mui/material/DialogContent';
import { ToastContainer, toast } from 'react-toastify';
import { format } from 'date-fns';
import PopupInterview from '../../../Admin/components/popupManage'
import { GetListCandidateApply, GetInterViewById, SetinterviewCandidate, SelectOptionInterviewByEmp, GetListCandidateInterviewByEid } from '../../../../Service/interviewService';
import { GetChangeStatus1, GetChangeStatus, GetReasonCancelByJobApplicationId } from '../../../../Service/jobApplicationService.js';
import { AddNotiForCandidate } from '../../../../Service/notificationService.js';
import { SentFeedbackToCandidate } from '../../../../Service/feedbackService';
function CandidateManage() {

    const [toggleTab, setToggleTab] = useState(1)
    const [notification, setNotification] = useState({ accountId: '', fromAccountId: '', title: '', content: '' })

    const toggleTabHandle = (index) => {
        setToggleTab(index)
    }
    // --------------------------------phỏng vấn-----------------------------------
    const [emid, setEmid] = useState(sessionStorage.getItem("employerId"));
    const [listJobInterview, setlistJobInterview] = useState([]);
    const [select, setSelect] = useState([]);
    const [inter, setinter] = useState();
    const [listJobWaiting, setListJobWaiting] = useState([]);
    const [listJobDone, setListJobDone] = useState([]);
    const [listJobDeny, setListJobDeny] = useState([]);
    const [listCandidatecance, setlistCandidatecance] = useState([]);
    const [status, setStatus] = useState()

    const [open3, setOpen3] = useState(false);
    const [open, setOpen] = useState(false);
    const [open1, setOpen1] = useState(false);
    const [run, setRun] = useState(false);
    const [rating, setRating] = useState(0);
    const [reasonHistory, setReasonHistory] = useState('');
    const [feedback, setFeedback] = useState({ content: '', from: '', to: '', status: 0, star: '' })
    const [Jobid, setJobid] = useState();
    const [MessageError, setMessageError] = useState('');

    const [btnPopup, setBtnPopup] = useState(false)

    const [currentPage, setCurrentPage] = useState(1);
    const recordPerPage = 2;
    const lastIndex = currentPage * recordPerPage;
    const firstIndex = lastIndex - recordPerPage;

    const [jobWattingSubArray, setJobWattingSubArray] = useState([]);
    const [jobDoneSubArray, setjobDoneSubArray] = useState([]);
    const [jobRejectSubArray, setjobRejectSubArray] = useState([]);
    const [jobCandidateCancelSubArray, setjobCandidateCancelSubArray] = useState([]);
    const [jobInterviewSubArray, setjobInterviewSubArray] = useState([]);
    const tokenE = localStorage.getItem("tokenE");
    useEffect(() => {
        Select();
        GetJobWaiting();
        GetJobDone();
        GetJobDeny();
        getJobInterview();
        getlistCandidatecance();
        const searchParams = new URLSearchParams(window.location.search);
        if (searchParams.get('toggleTab')) {
            if (!run) {
                setToggleTab(parseInt(searchParams.get('toggleTab'), 10));
                setRun(true);
            }
        }
    }, [status, rating])

    useEffect(() => {
        if (listJobWaiting.length > 0) {
            setJobWattingSubArray(listJobWaiting.filter(subArray => subArray.length > 0));
        }
    }, [listJobWaiting]);

    useEffect(() => {
        if (listJobInterview.length > 0) {
            setjobInterviewSubArray(listJobInterview.filter(subArray => subArray.length > 0));
        }
    }, [listJobInterview]);
    useEffect(() => {
        if (listJobDone.length > 0) {
            setjobDoneSubArray(listJobDone.filter(subArray => subArray.length > 0));
        }
    }, [listJobDone]);
    useEffect(() => {
        if (listJobDeny.length > 0) {
            setjobRejectSubArray(listJobDeny.filter(subArray => subArray.length > 0)); // Loại bỏ mảng rỗng
        }
    }, [listJobDeny]);
    useEffect(() => {
        if (listCandidatecance.length > 0) {
            setjobCandidateCancelSubArray(listCandidatecance.filter(subArray => subArray.length > 0));
        }
    }, [listCandidatecance]);

    const Select = async () => {
        let res = await SelectOptionInterviewByEmp(emid, tokenE);
        if (res) {
            setSelect(res);
        }
        // console.log("SelectOptionInterviewByEmp", res);
    }
    //---------------------------------Job Interview---------------------------------//
    const getJobInterview = async () => {
        let res = await GetListCandidateInterviewByEid(emid, 4, tokenE);
        if (res) {
            setlistJobInterview(res);
        }
        console.log("GetListCandidateInterviewByEid", res);
        // console.log("Jobdetai", res);
        // console.log("candiate", res);
    }

    const recordsJobInterview = jobInterviewSubArray.slice(firstIndex, lastIndex);
    const npageJobInterview = Math.ceil(jobInterviewSubArray.length / recordPerPage);
    const numbersJobInterview = [...Array(npageJobInterview + 1).keys()].slice(1);

    const nextPageJobInterview = () => {
        if (currentPage != npageJobInterview) {
            setCurrentPage(currentPage + 1)
        }
    }
    let countI = 0;
    // Duyệt qua từng phần tử của mảng recordsJobWaiting
    for (let i = 0; i < jobInterviewSubArray.length; i++) {
        // Lấy số phần tử của mảng con trong mỗi phần tử của recordsJobWaiting
        let numberOfElementsInSubarray = jobInterviewSubArray[i].length;
        countI += numberOfElementsInSubarray;
    }
    //---------------------------------Job Watting---------------------------------//
    const GetJobWaiting = async () => {
        let res = await GetListCandidateApply(emid, tokenE);
        if (res) {
            setListJobWaiting(res);
        }
        console.log("JobWaiting", res);
    }

    const recordsJobWatting = jobWattingSubArray.slice(firstIndex, lastIndex);
    const npageJobWatting = Math.ceil(jobWattingSubArray.length / recordPerPage);
    const numbersJobWatting = [...Array(npageJobWatting + 1).keys()].slice(1);

    const nextPageJobWatting = () => {
        if (currentPage != npageJobWatting) {
            setCurrentPage(currentPage + 1)
        }
    }
    let countW = 0;
    // Duyệt qua từng phần tử của mảng recordsJobWaiting
    for (let i = 0; i < jobWattingSubArray.length; i++) {
        // Lấy số phần tử của mảng con trong mỗi phần tử của recordsJobWaiting
        let numberOfElementsInSubarray = jobWattingSubArray[i].length;
        countW += numberOfElementsInSubarray;
    }

    //---------------------------------Job Done---------------------------------//

    const GetJobDone = async () => {
        let res = await GetListCandidateInterviewByEid(emid, 2, tokenE);
        if (res) {
            setListJobDone(res);
        }
        console.log("JobDone", res);
    }

    const recordsJobDone = jobDoneSubArray.slice(firstIndex, lastIndex);
    const npageJobDone = Math.ceil(jobDoneSubArray.length / recordPerPage);
    const numbersJobDone = [...Array(npageJobDone + 1).keys()].slice(1);

    const nextPageJobDone = () => {
        if (currentPage != npageJobDone) {
            setCurrentPage(currentPage + 1)
        }
    }
    let countD = 0;
    // Duyệt qua từng phần tử của mảng recordsJobWaiting
    for (let i = 0; i < jobDoneSubArray.length; i++) {
        // Lấy số phần tử của mảng con trong mỗi phần tử của recordsJobWaiting
        let numberOfElementsInSubarray = jobDoneSubArray[i].length;
        countD += numberOfElementsInSubarray;
    }
    //---------------------------------Job Reject---------------------------------//
    const GetJobDeny = async () => {
        let res = await GetListCandidateInterviewByEid(emid, 3, tokenE);
        if (res) {
            setListJobDeny(res);
        }
        console.log("Cance", res);
    }

    const recordsJobReject = jobRejectSubArray.slice(firstIndex, lastIndex);
    const npageJobReject = Math.ceil(jobRejectSubArray.length / recordPerPage);
    const numbersJobReject = [...Array(npageJobReject + 1).keys()].slice(1);

    const nextPageJobReject = () => {
        if (currentPage != npageJobReject) {
            setCurrentPage(currentPage + 1)
        }
    }
    let countR = 0;
    // Duyệt qua từng phần tử của mảng recordsJobWaiting
    for (let i = 0; i < jobRejectSubArray.length; i++) {
        // Lấy số phần tử của mảng con trong mỗi phần tử của recordsJobWaiting
        let numberOfElementsInSubarray = jobRejectSubArray[i].length;
        countR += numberOfElementsInSubarray;
    }
    //---------------------------------Candidate Cancel---------------------------------//
    const getlistCandidatecance = async () => {
        let res = await GetListCandidateInterviewByEid(emid, 1, tokenE);
        if (res) {
            setlistCandidatecance(res);
        }
        console.log("setlistCandidatecance", res);
    }

    const recordsJobCandidateCancel = jobCandidateCancelSubArray.slice(firstIndex, lastIndex);
    const npageJobCandidateCancel = Math.ceil(jobCandidateCancelSubArray.length / recordPerPage);
    const numbersJobCandidateCancel = [...Array(npageJobCandidateCancel + 1).keys()].slice(1);

    const nextPageJobCandidateCancel = () => {
        if (currentPage != npageJobCandidateCancel) {
            setCurrentPage(currentPage + 1)
        }
    }
    let countC = 0;
    // Duyệt qua từng phần tử của mảng recordsJobWaiting
    for (let i = 0; i < jobCandidateCancelSubArray.length; i++) {
        // Lấy số phần tử của mảng con trong mỗi phần tử của recordsJobWaiting
        let numberOfElementsInSubarray = jobCandidateCancelSubArray[i].length;
        countC += numberOfElementsInSubarray;
    }
    //-------------------------------------------------------------------------------------------//
    const PhongVan = async (jobApplicationId) => {
        const userConfirmed = window.confirm('Bạn có chắc muốn chuyển người này sang chế độ phỏng vấn không?');
        // Nếu người dùng đồng ý, thực hiện xóa
        if (userConfirmed) {
            let res = await GetChangeStatus(jobApplicationId, 4, tokenE);
            console.log("GetJobApplicationChangeStatus", res);
            if (res) {
                toast.success("Chuyển sang phỏng vấn thành công");
                setStatus(jobApplicationId);
            }
        }
    }
    const [CID, setCID] = useState("");
    const [TITLE, setTITLE] = useState("");
    const [JOBAPPLICATIONID, setJOBAPPLICATIONID] = useState("");
    const ChapNhan = async (cid, title, jobApplicationId) => {
        const userConfirmed = window.confirm('Bạn có chắc muốn nhận ứng viên này không?');
        // Nếu người dùng đồng ý, thực hiện xóa
        if (userConfirmed) {
            setCID(cid);
            setTITLE(title);
            setJOBAPPLICATIONID(jobApplicationId);
            let res = await GetChangeStatus(jobApplicationId, 2, tokenE);
            console.log("GetJobApplicationChangeStatus", res);
            if (res.message==="Quantity") {
                setOpen3(true);
            }else{
                notification.accountId = cid;
                notification.fromAccountId = sessionStorage.getItem('idOfEmp');
                notification.title = title + " thông báo";
                notification.content = `Bạn đã trúng tuyển công việc ${title}. Chi tiết công việc sẽ được thông báo sau bởi nhà tuyển dụng. Xin cảm ơn `;
                await AddNotiForCandidate(notification, tokenE);
                toast.success("Bạn đã nhận ứng viên thành công");
                setStatus(jobApplicationId);
            }
        }
    }

    const ChapNhanThem = async () => {
        // Nếu người dùng đồng ý, thực hiện xóa
        let res = await GetChangeStatus1(JOBAPPLICATIONID, 2, tokenE);
            console.log("GetJobApplicationChangeStatus", res);
            if (res) {
                notification.accountId = CID;
                notification.fromAccountId = sessionStorage.getItem('idOfEmp');
                notification.title = TITLE + " thông báo";
                notification.content = `Bạn đã trúng tuyển công việc ${TITLE}. Chi tiết công việc sẽ được thông báo sau bởi nhà tuyển dụng. Xin cảm ơn `;
                await AddNotiForCandidate(notification, tokenE);
                toast.success("Bạn đã nhận ứng viên thành công");
                setStatus(JOBAPPLICATIONID);
                setOpen3(false);
            }
    }
    const TuChoi = async (cid, title, jobApplicationId) => {
        const userConfirmed = window.confirm('Bạn có chắc muốn từ chối ứng viên này không?');
        // Nếu người dùng đồng ý, thực hiện xóa
        if (userConfirmed) {
            let res = await GetChangeStatus(jobApplicationId, 3, tokenE);
            console.log("GetJobApplicationChangeStatus", res);
            if (res) {
                notification.accountId = cid;
                notification.fromAccountId = sessionStorage.getItem('idOfEmp');
                notification.title = title + " thông báo";
                notification.content = `Công việc ${title} bạn ứng tuyển đã bị từ chối. Xin cảm ơn `;
                await AddNotiForCandidate(notification, tokenE);
                toast.success("Bạn đã từ chối ứng viên thành công");
                setStatus(jobApplicationId);
            }
        }
    }
    const [idcan, setidcan] = useState();
    const [idjob, setidjob] = useState();
    const [title, settitle] = useState();
    const [location, setlocation] = useState();
    const SetInterview = async (idcan, idjob, title, location) => {
        setidcan(idcan);
        setidjob(idjob);
        settitle(title);
        setlocation(location);
        setBtnPopup(true);
    }

    const AddInterview = async (idcan, idjob, title, location) => {
        console.log("cid InterviewId jobid", idcan, selectedValues[inter], idjob);
        let setInterview = await SetinterviewCandidate(idcan, selectedValues[inter], idjob);
        setStatus(idcan);
        console.log("SetinterviewCandidate", setInterview);
        let res = await GetInterViewById(setInterview.interviewId, tokenE);
        console.log("GetInterViewById", res);
        notification.accountId = idcan;
        notification.fromAccountId = sessionStorage.getItem('idOfEmp');
        notification.title = "Bạn có lịch phỏng vấn từ: " + title;
        notification.content = `Thời gian phỏng vấn: ${res.start}-${res.end} .Ngày: ${res.date}. Địa chỉ: ${location}`;
        await AddNotiForCandidate(notification, tokenE);
        console.log("selectedValues", selectedValues[inter]);
        setBtnPopup(false);
        toast.success("Tạo phỏng vấn thành công");
    }
    const [selectedValues, setSelectedValues] = useState(Array(select.length).fill(''));

    // Xử lý sự kiện khi giá trị của một Form.Select thay đổi
    const handleSelectChange = (event, index) => {
        // Tạo một bản sao của mảng state
        setinter(index);
        const newSelectedValues = [...selectedValues];
        // Cập nhật giá trị đã chọn cho Form.Select tại vị trí index
        newSelectedValues[index] = event.target.value;
        // Cập nhật state
        setSelectedValues(newSelectedValues);
    };

    // --------------------------------Chờ duyệt-----------------------------------


    // Thêm useState để lưu trữ thông tin lý do hủy và hiển thị Modal
    const [showReasonModal, setShowReasonModal] = useState(false);
    const [reasonCancel, setReasonCancel] = useState('');

    // Tạo hàm để hiển thị Modal và thiết lập thông tin lý do hủy
    const handleShowReasonModal = async (id) => {
        try {
            const reason = await GetReasonCancelByJobApplicationId(id, tokenE); // Đợi Promise được giải quyết
            setShowReasonModal(true);
            setReasonCancel(reason);
        } catch (error) {
            console.error('Error fetching reas  on:', error);
        }
    };

    // Tạo hàm để ẩn Modal
    const handleCloseReasonModal = () => {
        setShowReasonModal(false);
    };
    const handleClickOpenHistory = (jobid) => {
        setJobid(jobid);
        setOpen1(true);
    };

    const handleCloseHistory = () => {
        setReasonHistory("");
        setMessageError("");
        setRating(0);
        setOpen1(false);
    };
    const handleChangeHistory = (event) => {
        setReasonHistory(event.target.value);
    };
    const handleSubmitHistory = async (canid1, title) => {
        if (!reasonHistory.length > 0) {
            setMessageError("Mời bạn nhập nôi dung");
            return;
        }
        feedback.content = reasonHistory;
        feedback.from = emid;
        feedback.to = Jobid;
        feedback.star = rating;
        console.log(feedback);
        setStatus(Jobid);
        let res = await SentFeedbackToCandidate(feedback, tokenE);
        console.log("feedback",res);
        if (res.message === "successfull") {
            notification.accountId = canid1;
            notification.fromAccountId = parseInt(sessionStorage.getItem("idOfEmp"));
            notification.title = `Bạn đã nhận được đánh giá từ: ${title}`;
            notification.content = `Nội dung đánh giá: ${reasonHistory}. Đánh giá sao: ${rating} sao. Lưu ý: đánh giá sao thấp sẽ làm giới hạn số lần ứng tuyển của bạn!`;
            console.log("notification",notification);
            await AddNotiForCandidate(notification, tokenE);
            toast.success("Bạn đã đánh giá công việc ");
            // showToast("Bạn đã đánh giá công việc ");
            // Điều gì xảy ra khi bạn nhấn "Gửi" ở đây
            handleCloseHistory(); // Đóng popup sau khi gửi
        }

    }

    const prePage = () => {
        if (currentPage != 1) {
            setCurrentPage(currentPage - 1)
        }
    }

    const changePage = (id) => {
        setCurrentPage(id)
    }
    const handleClose = () => {
        setOpen3(false);
    };
    return (
        <>
            <Header />
            <div className="employer-page">
                <div className="employer-page-sidebar">
                    <SideBar />
                </div>

                <div className="employer-page-content">
                    <div className="post-btn-control">
                        <div className="post-btn-control-item">
                            <div className={toggleTab === 1 ? "post-btn post-btn-active" : "post-btn"} onClick={() => toggleTabHandle(1)}>Chờ duyệt({countW})</div>
                            <div className={toggleTab === 2 ? "post-btn post-btn-active" : "post-btn"} onClick={() => toggleTabHandle(2)}>Chờ phỏng vấn({countI})</div>
                            <div className={toggleTab === 3 ? "post-btn post-btn-active" : "post-btn"} onClick={() => toggleTabHandle(3)}>Đã tuyển({countD})</div>
                            <div className={toggleTab === 4 ? "post-btn post-btn-active" : "post-btn"} onClick={() => toggleTabHandle(4)}>Từ chối({countR})</div>
                            <div className={toggleTab === 5 ? "post-btn post-btn-active" : "post-btn"} onClick={() => toggleTabHandle(5)}>Ứng viên hủy({countC})</div>
                        </div>
                    </div>
                    {/* ---------------------------------------------------------------------------Chờ duyệt ------------------------------------------------------ */}
                    <div className={toggleTab === 1 ? "candidate-manage-item" : "candidate-manage-item none-active-post-content"}>
                        <div className="candidate-manage-item">
                            <div style={{ minHeight: 520 }}>
                                {recordsJobWatting && recordsJobWatting.length > 0 &&
                                    recordsJobWatting.map((item, index) => {
                                        return (
                                            (item && item.length > 0) ? (
                                                <div className="candidate-manage-item-job">
                                                    <div className="candidate-manage-item-job-name">
                                                        {item && item.length > 0 &&
                                                            item.slice(0, 1).map((item4, index3) => {
                                                                return (
                                                                    <a>{item4.job.title}({item.length})</a>
                                                                )
                                                            })}
                                                    </div>
                                                    <div className="candidate-manage-list-cv">
                                                        {item && item.length > 0 &&
                                                            item.map((item2, index2) => {
                                                                return (
                                                                    <div className="candidate-manage-item-job-cv">
                                                                        <div className='job-list-item'>
                                                                            <div className='candidate-manage-item-left'>
                                                                                <div className="job-list-logo">
                                                                                    <img id='company-logo' src={item2.applicant.image} alt="" />
                                                                                </div>
                                                                                <div className='job-list-item-left-content'>
                                                                                    <div className='candidate-manage-item-job-cv-name'><a style={{color: '#000'}} href={`/cvGenarel?candidateId=${item2.applicantId}`}>{item2.applicant.account.fullName}</a></div>
                                                                                    <div className='job-list-des'>
                                                                                        <div className='candidate-job-list-company'><span><i class="fa-solid fa-location-dot"></i> Địa chỉ:</span> {item2.applicant.distric}-{item2.applicant.city}</div>
                                                                                    </div>
                                                                                    <div className='job-list-des'>
                                                                                        <div className='candidate-job-list-company'><span><i class="fa-solid fa-cake-candles"></i> Ngày sinh:</span> {format(new Date(item2.applicant.dob), 'dd-MM-yyyy')}</div>
                                                                                    </div>
                                                                                    <div className='job-list-des'>
                                                                                        <div className='candidate-job-list-company'><span><i class="fa-solid fa-phone"></i> Liên hệ:</span> {item2.applicant.phone}</div>
                                                                                    </div>
                                                                                </div>
                                                                            </div>

                                                                            <div className='candidate-manage-item-right'>
                                                                                <div className='candidate-manage-item-right-content'>
                                                                                    {/* <a class="icon-container"><i onClick={() => PhongVan(item2.id, 4)} id='candidate-manage-cv-icon' class="fa-solid fa-user-check"></i></a>
                                                                                    <a class="icon-container"><i onClick={() => ChapNhan(item2.applicantId, item2.job.title, item2.id, 2)} id='candidate-manage-cv-icon' class="fa-solid fa-comment"></i></a>
                                                                                    <a class="icon-container"><i onClick={() => TuChoi(item2.applicantId, item2.job.title, item2.id, 3)} id='candidate-manage-cv-icon' class="fa-solid fa-ban"></i></a> */}
                                                                                    {item2.status === 5 ? (
                                                                                        <Button id="btn-manage-can" disabled variant="info" >Đang phỏng vấn</Button>
                                                                                    ) : (
                                                                                        <Button id="btn-manage-can" onClick={() => PhongVan(item2.id, 4)} variant="info" >Chờ phỏng vấn</Button>
                                                                                    )}
                                                                                    <Button id="btn-manage-can" onClick={() => ChapNhan(item2.applicantId, item2.job.title, item2.id, 2)} variant="success">Chấp nhận</Button>
                                                                                    <Button id="btn-manage-can" onClick={() => TuChoi(item2.applicantId, item2.job.title, item2.id, 3)} variant="danger">Từ chối</Button>
                                                                                </div>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                )
                                                            })}
                                                    </div>
                                                </div>

                                            ) : (null)
                                        )
                                    })}
                                <Dialog open={open3} onClose={handleClose} className="custom-dialog-content">
                                    <div style={{ display: 'flex', alignItems: 'center', margin: 50, fontSize: 20 }}>
                                        <span>Bạn đã tuyển đủ số lượng cần tuyển cho công việc này. Bạn có muốn tuyển thêm không?</span>
                                    </div>
                                    <DialogActions>
                                        <Button onClick={handleClose} color="primary">
                                            Đóng
                                        </Button>
                                        <Button onClick={ChapNhanThem} color="primary">
                                            Tuyển
                                        </Button>
                                    </DialogActions>
                                </Dialog>
                            </div>

                            {recordsJobWatting && recordsJobWatting.length > 0 &&
                                <nav style={{ textAlign: 'center', marginLeft: 60 }}>
                                    <ul className="pagination">
                                        <li className="page-item">
                                            <a className="page-link" onClick={prePage}>Prev</a>
                                        </li>
                                        {
                                            numbersJobWatting.map((n, i) => (
                                                <li className={`page-item ${currentPage === n ? 'active' : ''}`} key={i}>
                                                    <a className="page-link" onClick={() => changePage(n)}>{n}</a>
                                                </li>
                                            ))
                                        }
                                        <li className="page-item">
                                            <a className="page-link" onClick={nextPageJobWatting}>Next</a>
                                        </li>
                                    </ul>
                                </nav>
                            }
                        </div>

                    </div>

                    {/* ---------------------------------------------------------------------------Phỏng vấn ------------------------------------------------------ */}
                    <div className={toggleTab === 2 ? "candidate-interview" : "candidate-interview none-active-post-content"}>
                        <div className="candidate-manage-item">
                            <div style={{ minHeight: 520 }}>
                                {recordsJobInterview && recordsJobInterview.length > 0 &&
                                    recordsJobInterview.map((item, index) => {
                                        return (
                                            (item && item.length > 0) ? (
                                                <div className="candidate-manage-item-job">
                                                    <div className="candidate-manage-item-job-name">
                                                        {item && item.length > 0 &&
                                                            item.slice(0, 1).map((item4, index3) => {
                                                                return (
                                                                    <a>{item4.job.title}({item.length})</a>
                                                                )
                                                            })}
                                                    </div>
                                                    <div className="candidate-manage-list-cv">
                                                        {item && item.length > 0 &&
                                                            item.map((item2, index2) => {
                                                                return (
                                                                    <div className="candidate-manage-item-job-cv">
                                                                        <div className='job-list-item'>
                                                                            <div className='candidate-manage-item-left'>
                                                                                <div className="job-list-logo">
                                                                                    <img id='company-logo' src={item2.applicant.image} alt="" />
                                                                                </div>
                                                                                <div className='job-list-item-left-content'>
                                                                                    <div className='candidate-manage-item-job-cv-name'><a style={{color: '#000'}} href={`/cvGenarel?candidateId=${item2.applicantId}`}>{item2.applicant.account.fullName}</a></div>
                                                                                    <div className='job-list-des'>
                                                                                        <div className='candidate-job-list-company'><span><i class="fa-solid fa-location-dot"></i> Địa chỉ:</span> {item2.applicant.distric}-{item2.applicant.city}</div>
                                                                                    </div>
                                                                                    <div className='job-list-des'>
                                                                                        <div className='candidate-job-list-company'><span><i class="fa-solid fa-cake-candles"></i> Ngày sinh:</span> {format(new Date(item2.applicant.dob), 'dd-MM-yyyy')}</div>
                                                                                    </div>
                                                                                    <div className='job-list-des'>
                                                                                        <div className='candidate-job-list-company'><span><i class="fa-solid fa-phone"></i> Liên hệ:</span> {item2.applicant.phone}</div>
                                                                                    </div>
                                                                                </div>
                                                                            </div>

                                                                            <div className='candidate-manage-item-right'>
                                                                                <div className='candidate-manage-item-right-content'>
                                                                                    <Button id="btn-manage-can" variant="secondary" onClick={() => SetInterview(item2.applicantId, item2.jobId, item2.job.title, item2.job.location)}>Đặt lịch phỏng vấn</Button>
                                                                                    <PopupInterview trigger={btnPopup} setTrigger={setBtnPopup}>
                                                                                        <div className='admin-notify-top'>Đặt lịch phỏng vấn</div>
                                                                                        <div className="candidate-cancel-btn">
                                                                                            <Form.Select
                                                                                                aria-label="Default select example"
                                                                                                id={`interview-select-${index}${index2}`}
                                                                                                onChange={(e) => handleSelectChange(e, index + "" + index2)}
                                                                                                value={selectedValues[index + "" + index2]}>
                                                                                                <option style={{ display: "none" }}>Chọn Lịch</option>
                                                                                                {select && select.length > 0 &&
                                                                                                    select.map((item1, index1) => {
                                                                                                        return (
                                                                                                            <option value={item1.id}>Mã lịch : #{item1.id}</option>
                                                                                                        )
                                                                                                    })}
                                                                                            </Form.Select>
                                                                                            <div className="candidate-popup-btn">
                                                                                                <Button style={{ float: 'inline-end', marginLeft: '10px' }} variant="success" onClick={() => AddInterview(idcan, idjob, title, location)}>Tạo lịch phỏng vấn</Button>
                                                                                                <Button onClick={() => setBtnPopup(false)} style={{ float: 'inline-end', marginLeft: '10px' }} variant="danger">Hủy bỏ</Button>
                                                                                            </div>
                                                                                        </div>
                                                                                        {/* <Button style={{ float: 'inline-end' }} onClick={() => handleReasoncancelClose()} variant="success">Phản hồi</Button> */}
                                                                                    </PopupInterview>
                                                                                </div>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                )
                                                            })}
                                                    </div>
                                                </div>
                                            ) : (null)
                                        )
                                    })}
                            </div>
                            {recordsJobInterview && recordsJobInterview.length > 0 &&
                                <nav style={{ textAlign: 'center', marginLeft: 60 }}>
                                    <ul className="pagination">
                                        <li className="page-item">
                                            <a className="page-link" onClick={prePage}>Prev</a>
                                        </li>
                                        {
                                            numbersJobInterview.map((n, i) => (
                                                <li className={`page-item ${currentPage === n ? 'active' : ''}`} key={i}>
                                                    <a className="page-link" onClick={() => changePage(n)}>{n}</a>
                                                </li>
                                            ))
                                        }
                                        <li className="page-item">
                                            <a className="page-link" onClick={nextPageJobInterview}>Next</a>
                                        </li>
                                    </ul>
                                </nav>
                            }
                        </div>
                    </div>

                    <div className={toggleTab === 3 ? "candidate-recruited" : "candidate-recruited none-active-post-content"}>
                        <div className="candidate-manage-item">
                            <div style={{ minHeight: 520 }}>
                                {recordsJobDone && recordsJobDone.length > 0 &&
                                    recordsJobDone.map((item, index) => {
                                        return (
                                            (item && item.length > 0) ? (
                                                <div className="candidate-manage-item-job">
                                                    <div className="candidate-manage-item-job-name">
                                                        {item && item.length > 0 &&
                                                            item.slice(0, 1).map((item4, index3) => {
                                                                return (
                                                                    <a>{item4.job.title}({item.length})</a>
                                                                )
                                                            })}
                                                    </div>
                                                    <div className="candidate-manage-list-cv">
                                                        {item && item.length > 0 &&
                                                            item.slice(0, 4).map((item2, index2) => {
                                                                return (
                                                                    <div className="candidate-manage-item-job-cv">
                                                                        <div className='job-list-item'>
                                                                            <div className='candidate-manage-item-left'>
                                                                                <div className="job-list-logo">
                                                                                    <img id='company-logo' src={item2.applicant.image} alt="" />
                                                                                </div>
                                                                                <div className='job-list-item-left-content'>
                                                                                    <div className='candidate-manage-item-job-cv-name'><a style={{color: '#000'}} href={`/cvGenarel?candidateId=${item2.applicantId}`}>{item2.applicant.account.fullName}</a></div>
                                                                                    <div className='job-list-des'>
                                                                                        <div className='candidate-job-list-company'><span><i class="fa-solid fa-location-dot"></i> Địa chỉ:</span> {item2.applicant.distric}-{item2.applicant.city}</div>
                                                                                    </div>
                                                                                    <div className='job-list-des'>
                                                                                        <div className='candidate-job-list-company'><span><i class="fa-solid fa-cake-candles"></i> Ngày sinh:</span> {format(new Date(item2.applicant.dob), 'dd-MM-yyyy')}</div>
                                                                                    </div>
                                                                                    <div className='job-list-des'>
                                                                                        <div className='candidate-job-list-company'><span><i class="fa-solid fa-phone"></i> Liên hệ:</span> {item2.applicant.phone}</div>
                                                                                    </div>
                                                                                </div>
                                                                            </div>

                                                                            <div className='candidate-manage-item-right'>
                                                                                <div className='candidate-manage-item-right-content'>
                                                                                    {item2.isEmployerComment === 1 ? (
                                                                                        <div className='job-list-item-right'>
                                                                                            <Button disabled id="btn-manage-can">Đã Feedback</Button>
                                                                                        </div>
                                                                                    ) : (
                                                                                        <div className='job-list-item-right'>
                                                                                            <Button onClick={() => handleClickOpenHistory(item2.id)} id="btn-manage-can">Feedback</Button>
                                                                                            <Dialog open={open1} onClose={handleCloseHistory} className="custom-dialog-content">
                                                                                                <img src="data:image/png;base64,"
                                                                                                    alt="Logo" style={{ width: '100%' }} />
                                                                                                <DialogContent className="custom-dialog-content-container">
                                                                                                    <textarea
                                                                                                        value={reasonHistory}
                                                                                                        onChange={handleChangeHistory}
                                                                                                        placeholder="Nhập nội dung feedback" style={{ width: '400px', height: '200px', opacity: '0.7', fontWeight: 'bold' }}></textarea>
                                                                                                </DialogContent>
                                                                                                <div style={{ display: 'flex', alignItems: 'center', marginLeft: 50 }}>
                                                                                                    <span>Đánh giá sao:</span>
                                                                                                    <Rating
                                                                                                        value={rating}
                                                                                                        onChange={(event, newValue) => {
                                                                                                            setRating(newValue);
                                                                                                        }}
                                                                                                    />
                                                                                                </div>
                                                                                                <DialogActions>
                                                                                                    <Button onClick={handleCloseHistory} color="primary">
                                                                                                        Đóng
                                                                                                    </Button>
                                                                                                    <Button onClick={() => handleSubmitHistory(item2.applicantId, item2.job.title)} color="primary">
                                                                                                        Gửi
                                                                                                    </Button>
                                                                                                </DialogActions>
                                                                                                <p className="error-message">{MessageError}</p>
                                                                                            </Dialog>
                                                                                        </div>
                                                                                    )}
                                                                                </div>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                )
                                                            })}
                                                    </div>
                                                </div>
                                            ) : (null)
                                        )
                                    })}
                            </div>
                            {recordsJobDone && recordsJobDone.length > 0 &&
                                <nav style={{ textAlign: 'center', marginLeft: 60 }}>
                                    <ul className="pagination">
                                        <li className="page-item">
                                            <a className="page-link" onClick={prePage}>Prev</a>
                                        </li>
                                        {
                                            numbersJobDone.map((n, i) => (
                                                <li className={`page-item ${currentPage === n ? 'active' : ''}`} key={i}>
                                                    <a className="page-link" onClick={() => changePage(n)}>{n}</a>
                                                </li>
                                            ))
                                        }
                                        <li className="page-item">
                                            <a className="page-link" onClick={nextPageJobDone}>Next</a>
                                        </li>
                                    </ul>
                                </nav>
                            }
                        </div>
                    </div>
                    {/* ---------------------------------------------------------------------------Từ chối ------------------------------------------------------ */}
                    <div className={toggleTab === 4 ? "candidate-reject" : "candidate-reject none-active-post-content"}>
                        <div className="candidate-manage-item">
                            <div style={{ minHeight: 520 }}>
                                {recordsJobReject && recordsJobReject.length > 0 &&
                                    recordsJobReject.map((item, index) => {
                                        return (
                                            (item && item.length > 0) ? (
                                                <div className="candidate-manage-item-job">
                                                    <div className="candidate-manage-item-job-name">
                                                        {item && item.length > 0 &&
                                                            item.slice(0, 1).map((item4, index3) => {
                                                                return (
                                                                    <a>{item4.job.title}({item.length})</a>
                                                                )
                                                            })}
                                                    </div>
                                                    <div className="candidate-manage-list-cv">
                                                        {item && item.length > 0 &&
                                                            item.map((item2, index2) => {
                                                                return (
                                                                    <div className="candidate-manage-item-job-cv">
                                                                        <div className='job-list-item'>
                                                                            <div className='candidate-manage-item-left'>
                                                                                <div className="job-list-logo">
                                                                                    <img id='company-logo' src={item2.applicant.image} alt="" />
                                                                                </div>
                                                                                <div className='job-list-item-left-content'>
                                                                                    <div className='candidate-manage-item-job-cv-name'><a style={{color: '#000'}} href={`/cvGenarel?candidateId=${item2.applicantId}`}>{item2.applicant.account.fullName}</a></div>
                                                                                    <div className='job-list-des'>
                                                                                        <div className='candidate-job-list-company'><span><i class="fa-solid fa-location-dot"></i> Địa chỉ:</span> {item2.applicant.distric}-{item2.applicant.city}</div>
                                                                                    </div>
                                                                                    <div className='job-list-des'>
                                                                                        <div className='candidate-job-list-company'><span><i class="fa-solid fa-cake-candles"></i> Ngày sinh:</span> {format(new Date(item2.applicant.dob), 'dd-MM-yyyy')}</div>
                                                                                    </div>
                                                                                    <div className='job-list-des'>
                                                                                        <div className='candidate-job-list-company'><span><i class="fa-solid fa-phone"></i> Liên hệ:</span> {item2.applicant.phone}</div>
                                                                                    </div>
                                                                                </div>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                )
                                                            })}
                                                    </div>
                                                </div>
                                            ) : (null)
                                        )
                                    })}
                            </div>
                            {recordsJobReject && recordsJobReject.length > 0 &&
                                <nav style={{ textAlign: 'center', marginLeft: 60 }}>
                                    <ul className="pagination">
                                        <li className="page-item">
                                            <a className="page-link" onClick={prePage}>Prev</a>
                                        </li>
                                        {
                                            numbersJobReject.map((n, i) => (
                                                <li className={`page-item ${currentPage === n ? 'active' : ''}`} key={i}>
                                                    <a className="page-link" onClick={() => changePage(n)}>{n}</a>
                                                </li>
                                            ))
                                        }
                                        <li className="page-item">
                                            <a className="page-link" onClick={nextPageJobReject}>Next</a>
                                        </li>
                                    </ul>
                                </nav>
                            }
                        </div>
                    </div>
                    {/* ---------------------------------------------------------------------------Đã tuyển ------------------------------------------------------ */}


                    <div className={toggleTab === 5 ? "candidate-cancel" : "candidate-cancel none-active-post-content"}>
                        <div className="candidate-manage-item">
                            <div style={{ minHeight: 520 }}>
                                {recordsJobCandidateCancel && recordsJobCandidateCancel.length > 0 &&
                                    recordsJobCandidateCancel.map((item, index) => {
                                        return (
                                            (item && item.length > 0) ? (
                                                <div className="candidate-manage-item-job">
                                                    <div className="candidate-manage-item-job-name">
                                                        {item && item.length > 0 &&
                                                            item.slice(0, 4).slice(0, 1).map((item4, index3) => {
                                                                return (
                                                                    <a>{item4.job.title}({item.length})</a>
                                                                )
                                                            })}
                                                    </div>
                                                    <div className="candidate-manage-list-cv">
                                                        {item && item.length > 0 &&
                                                            item.map((item2, index2) => {
                                                                return (
                                                                    <div className="candidate-manage-item-job-cv">
                                                                        <div className='job-list-item'>
                                                                            <div className='candidate-manage-item-left'>
                                                                                <div className="job-list-logo">
                                                                                    <img id='company-logo' src={item2.applicant.image} alt="" />
                                                                                </div>
                                                                                <div className='job-list-item-left-content'>
                                                                                    <div className='candidate-manage-item-job-cv-name'><a style={{color: '#000'}} href={`/cvGenarel?candidateId=${item2.applicantId}`}>{item2.applicant.account.fullName}</a></div>
                                                                                    <div className='job-list-des'>
                                                                                        <div className='candidate-job-list-company'><span><i class="fa-solid fa-location-dot"></i> Địa chỉ:</span> {item2.applicant.distric}-{item2.applicant.city}</div>
                                                                                    </div>
                                                                                    <div className='job-list-des'>
                                                                                        <div className='candidate-job-list-company'><span><i class="fa-solid fa-cake-candles"></i> Ngày sinh:</span> {format(new Date(item2.applicant.dob), 'dd-MM-yyyy')}</div>
                                                                                    </div>
                                                                                    <div className='job-list-des'>
                                                                                        <div className='candidate-job-list-company'><span><i class="fa-solid fa-phone"></i> Liên hệ:</span> {item2.applicant.phone}</div>
                                                                                    </div>
                                                                                </div>
                                                                            </div>

                                                                            <div className='candidate-manage-item-right'>
                                                                                <div className='candidate-manage-item-right-content'>
                                                                                    <div className="candidate-cancel-btn">
                                                                                        <div>
                                                                                            <Button
                                                                                                onClick={() => handleShowReasonModal(item2.id)}
                                                                                                id="btn-manage-can">Xem lý do</Button>
                                                                                        </div>
                                                                                    </div>
                                                                                </div>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                )
                                                            })}
                                                    </div>
                                                </div>
                                            ) : (null)
                                        )
                                    })}
                            </div>
                            {recordsJobCandidateCancel && recordsJobCandidateCancel.length > 0 &&
                                <nav style={{ textAlign: 'center', marginLeft: 60 }}>
                                    <ul className="pagination">
                                        <li className="page-item">
                                            <a className="page-link" onClick={prePage}>Prev</a>
                                        </li>
                                        {
                                            numbersJobCandidateCancel.map((n, i) => (
                                                <li className={`page-item ${currentPage === n ? 'active' : ''}`} key={i}>
                                                    <a className="page-link" onClick={() => changePage(n)}>{n}</a>
                                                </li>
                                            ))
                                        }
                                        <li className="page-item">
                                            <a className="page-link" onClick={nextPageJobCandidateCancel}>Next</a>
                                        </li>
                                    </ul>
                                </nav>
                            }
                        </div>
                    </div>
                </div>
            </div>

            <Modal show={showReasonModal} onHide={handleCloseReasonModal}>
                <Modal.Header closeButton>
                    <Modal.Title>Lý do hủy</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <p>{reasonCancel}</p>
                </Modal.Body>
                <Modal.Footer>
                    <Button variant="secondary" onClick={handleCloseReasonModal}>
                        Đóng
                    </Button>
                </Modal.Footer>
            </Modal>
            <ToastContainer />
        </>
    )
}
export default CandidateManage;